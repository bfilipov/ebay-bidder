        ...
        for each in searchItems:        
            ...
            try:
                if not(driver.find_elements_by_xpath("//*[contains(text(), 're the highest bidder.')]")):
                    driver.get(driver.find_element_by_id('bidBtn_btn').get_attribute("href"))
                    if not(driver.find_elements_by_xpath("//*[contains(text(), 'Your max bid:')]")):
                        driver.implicitly_wait(2)
                        act = webdriver.ActionChains(driver)
                        act.move_to_element(driver.find_element_by_id("MaxBidId")).click().send_keys(str(bidAmmount))
                        act.perform()
                        
                        driver.implicitly_wait(2)
                        act.move_to_element(driver.find_element_by_id("bidBtn_btn")).click()
                        act.perform()   
                       
                        driver.implicitly_wait(2)
                        act.move_to_element(driver.find_element_by_id("but_v4-1")).click()
                        act.perform()                
                        #driver.find_element_by_id("but_v4-1").click()

                        driver.implicitly_wait(2)
                        act.move_to_element(driver.find_element_by_id("but_v4-2")).click()
                        act.perform()
